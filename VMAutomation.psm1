function New-AutoDeployVM {
    ...
    Param(
        ...
        [IPAddress]$SubnetAddress="172.16.1.0",
...
function Remove-AutoDeployVM {
    ...
    Get-ADComputer $Name | Remove-ADComputer -Confirm:$false
}
...
function New-DebianVM {
    ...
    Param(
        ...
        # Notes that can be added to the VM.  
        [string]$Notes="AutoGenerated Debian VM"
    )

    $VMDir = "$Path\$Name"
    New-Item -Path $VMDir -ItemType Directory
    New-Item -Path "$VMDir\VM" -ItemType Directory
    Copy-Item -Path "F:\Images\Debian\DebianBase.vhdx" -Destination "$VMDir\$Name.vhdx"
    ...
    Add-VMHardDiskDrive -VMName $Name -ComputerName $VMHost -Path 'F:\Images\EDK II\EDK II.vhdx'

    $VMEfiDrive = Get-VMHardDiskDrive -VMName test -ControllerNumber 0 -ControllerLocation 1
    Set-VMFirmware -VMName $Name -ComputerName $VMHost -EnableSecureBoot Off -FirstBootDevice $VMEfiDrive

    # Hyper-V generates a semi-random MacAddress at first boot
    Start-VM -ComputerName $VMHost -Name $Name

    Start-Sleep -Seconds 7
    $VMCS = Get-WmiObject -Namespace root\virtualization\v2 -Class Msvm_ComputerSystem -Filter "ElementName='$($Name)'"
    $keyboard = $VMCS.GetRelated("Msvm_Keyboard")
    $keyboard.TypeText("bcfg boot add 0 fs0:\EFI\debian\shimx64.efi DEBIAN")
    $Keyboard.TypeKey("13")
    ...
    Set-VMFirmware -VMName $Name -ComputerName $VMHost -EnableSecureBoot On -SecureBootTemplate "MicrosoftUEFICertificateAuthority"
    Remove-VMHardDiskDrive -VMName $Name -ComputerName $VMHost -ControllerType SCSI -ControllerNumber 0 -ControllerLocation 1
